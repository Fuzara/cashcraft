...
import type { _SERVICE as CashcraftService } from "@declarations/cashcraft/service.did";
import type { _SERVICE as WalletsService } from "@declarations/wallets/service.did";
...
const getCanisterId = (canisterName: CanisterName): string => {
  ...
  if (!canisterId) {
    const errorMessage = `[Actor Creation Error] Canister ID for '${canisterName}' is undefined.
    - Environment variable ${envKey} is not set.
    - Please check your 'frontend/.env.local' file.
    - Run 'scripts/heal_env_and_declarations.sh' to regenerate the environment file.`;
